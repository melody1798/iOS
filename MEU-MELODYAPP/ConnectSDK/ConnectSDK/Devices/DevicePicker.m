...
- (void) showPicker:(id)sender
{
    ...
    if ([_devices count] > 0) {
        pickerTitle = [[NSBundle mainBundle] localizedStringForKey:@"Connect_SDK_Search_Title" value:@"Pick a device" table:@"ConnectSDK"];
    }
    else
    {
        pickerTitle = [[NSBundle mainBundle] localizedStringForKey:@"Connect_SDK_Search_Title" value:@"No device found" table:@"ConnectSDK"];
    }
...
- (void) showNavigation
{
  ...
  //  [mainWindow.rootViewController addChildViewController:_navigationController];
}
...
- (void)discoveryManager:(DiscoveryManager *)manager didFindDevice:(ConnectableDevice *)device
{
    if (_devices)
    {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (_actionSheet) {
                pickerTitle = [[NSBundle mainBundle] localizedStringForKey:@"Connect_SDK_Search_Title" value:@"Pick a device" table:@"ConnectSDK"];
                _actionSheet.title = pickerTitle;
            }
        });
    }
}
