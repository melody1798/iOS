...
@interface INFScrollView() <UIGestureRecognizerDelegate>
...
//    self.scrollEnabled = YES;
    ...
    //"Cancellable Content Touches" and "Delays Content Touches"
...
#define MAX_CONTENT_OFFSET_PROPORTION       (.25)
    ...
    if (newXOffset != currentOffset.x || newYOffset != currentOffset.y)
    {
       ...
       // self.contentOffset = CGPointMake(newXOffset, newYOffset);
        ...
        [self setContentOffset:CGPointMake(newXOffset, newYOffset) animated:NO];
...
- (void)doInitialTileSetup
{
    ...
    [_tileContainer setTag:100000];
    [self addSubview:self.tileContainer];
    [self setContentOffset:CGPointMake((self.tileContainer.bounds.size.width - self.bounds.size.width)/2, (self.tileContainer.bounds.size.height - self.bounds.size.height)/2) animated:NO];
}
